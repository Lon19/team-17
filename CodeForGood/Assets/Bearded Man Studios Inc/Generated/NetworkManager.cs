...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] ChatC4gManagerNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is ChatC4gManagerNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (ChatC4gManagerNetworkObject.Length > 0 && ChatC4gManagerNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(ChatC4gManagerNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<ChatC4gManagerBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiateChatC4gManager instead, its shorter and easier to type out ;)")]
		public ChatC4gManagerBehavior InstantiateChatC4gManagerNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(ChatC4gManagerNetworkObject[index]);
			var netBehavior = go.GetComponent<ChatC4gManagerBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<ChatC4gManagerBehavior>().networkObject = (ChatC4gManagerNetworkObject)obj;
		...
		/// <summary>
		/// Instantiate an instance of ChatC4gManager
		/// </summary>
		/// <returns>
		/// A local instance of ChatC4gManagerBehavior
		/// </returns>
		/// <param name="index">The index of the ChatC4gManager prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public ChatC4gManagerBehavior InstantiateChatC4gManager(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(ChatC4gManagerNetworkObject[index]);
			var netBehavior = go.GetComponent<ChatC4gManagerBehavior>();
